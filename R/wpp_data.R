...
# reg_name, reg_code and area_name, area_code: vectors with UN regions and continents
	...
	tfr_data <- read.UNtfr(wpp.year=wpp.year, my.tfr.file=my.tfr.file, 
								present.year=present.year, verbose=verbose)$tfr_data
...
read.UNtfr <- function(wpp.year, my.tfr.file=NULL, ...) {
	...
	return(list(tfr_data=result$data, replaced=result$replaced, added=result$added))
}

read.UNlocations <- function(tfr_data, wpp.year, package="bayesTFR", verbose=FALSE) {
	...
	nr_tfr_outcomes <- length(tfr_data[,1])
	...
	for (i in 1:nr_tfr_outcomes){
 		loc_index <- (1:n_loc_data)[loc_data$country_code == tfr_data$country_code[i]]
 		if (length(loc_index)<=0)
 			stop('Country ', tfr_data$country_code[i], ' not found in the file ', loc.file.name)
 		incl.code <- if(tfr_data$include_code[i] >= 0) tfr_data$include_code[i] else loc_data$include_code[loc_index]
...
set.wpp.extra <- function(meta, countries=NULL, my.tfr.file=NULL, verbose=FALSE) {
	...
	ncountries <- meta$regions$nr_countries
	...
	replaced.processed.est <- country.codes.processed[idx.replaced.processed[1:meta$nr_countries_estimation]]
	...
	added.processed.est <- country.codes.processed[idx.added.processed[1:meta$nr_countries_estimation]]
	...
	countries.processed.est <- country.codes.processed[idx.countries.processed[1:meta$nr_countries_estimation]]
	...
	if(length(include.codes) > 0) {
		include <- is.element(tfrs$tfr_data$country_code, include.codes)
		locations <- read.UNlocations(tfrs$tfr_data, wpp.year=meta$wpp.year, verbose=verbose)
		TFRmatrix.regions <- get.TFRmatrix.and.regions(tfrs$tfr_data[include,], locations$loc_data, 
